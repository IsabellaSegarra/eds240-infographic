...
micro_plastics_clean %>%
  ...
  filter(!is.na(morphological_category), !is.na(sample_medium)) %>%
  ggplot() +
  geom_mosaic(aes(x = product(sample_medium), fill = morphological_category), offset = 0.015) +
  scale_fill_manual(values = morphology_pal) +
  labs(x = "Source", y = NULL, fill = "Plastic\nMorphology",
       title = "Common plastic morphologies within the San Francisco Bay Estuary",
       subtitle = "") +
  theme_void() +
...
plot_2
```
3. Most common morphology and color combinations for plastic. 
```{r}
